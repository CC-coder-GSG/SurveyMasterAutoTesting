...
rem ---- Clean old results (avoid stale output.xml) ----
...
for /l %%i in (1,1,12) do (
  rem Use get-state (most reliable)
...
rem Kill by Port (PowerShell with fallback to netstat)
powershell -NoProfile -NonInteractive -Command ^
  "try { $c = Get-NetTCPConnection -LocalPort %APPIUM_PORT% -State Listen -ErrorAction Stop; if($c){ Stop-Process -Id $c.OwningProcess -Force -ErrorAction SilentlyContinue } } catch { exit 0 }"
for /f "tokens=5" %%p in ('netstat -ano ^| findstr /R /C:":%APPIUM_PORT% .*LISTENING"') do taskkill /F /PID %%p >nul 2>&1
...
for /l %%i in (1,1,30) do (
  powershell -NoProfile -NonInteractive -Command ^
    "try { if(Get-NetTCPConnection -LocalPort %APPIUM_PORT% -State Listen -ErrorAction Stop){ exit 0 } else { exit 1 } } catch { exit 2 }"
  if !errorlevel! EQU 0 (
    set "APPIUM_UP=1"
    goto :APPIUM_OK
  )
  rem fallback netstat (in case Get-NetTCPConnection is unavailable)
  netstat -ano | findstr /R /C:":%APPIUM_PORT% .*LISTENING" >nul && (set "APPIUM_UP=1" & goto :APPIUM_OK)
  timeout /t 1 /nobreak >nul
)

echo [ERROR] Appium failed to start on %APPIUM_PORT%.
if exist "%APPIUM_LOG%" powershell -NoProfile -NonInteractive -Command "Get-Content -Tail 80 '%APPIUM_LOG%'"
goto :FAIL
